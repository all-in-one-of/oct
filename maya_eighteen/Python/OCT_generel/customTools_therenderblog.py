...
def UI():
	...
	widgets['LightBtn'] = cmds.symbolButton(w=50,h=50,image='aiLight.png',command=lightFromView)
	widgets['QuadDescription'] = cmds.text( label='Light from view',h=20,fn='obliqueLabelFont')

	widgets['IsoBtn'] = cmds.symbolButton(w=50,h=50,image='iso.png',command=isoTextured)
	widgets['IsoTextDes'] = cmds.text( label='Isolate Textured',h=20,fn='obliqueLabelFont')

	widgets['OverrideBtn'] = cmds.symbolButton(w=50,h=50,image='override.png',command=MatOver)
	...
	widgets['SkyDomeBtn'] = cmds.symbolButton(w=50,h=50,image='aiSkyDomeLight.png',command=aiSkyDome)
	widgets['SkyDomeDescription'] = cmds.text( label='Skydome Light',h=20,fn='obliqueLabelFont')

	widgets['sceneBtn'] = cmds.symbolButton(w=50,h=50,image='aiscene.png',command=aiScene)
	widgets['sceneDescription'] = cmds.text( label='Test aiScene',h=20,fn='obliqueLabelFont')

	widgets['autoMapBtn'] = cmds.symbolButton(w=50,h=50,image='remaptext.png',command=autoMap)
	...
	widgets['FresnelImg'] = cmds.image( image='fresnel.png',w=300,h=43 )
	...
	widgets['renderBtn'] = cmds.symbolButton(w=30,h=30,image='ai_logo.png',command=setRender)
	...
	# Show Dock Window
	widgets['dockPanel'] = cmds.dockControl('toolDock',label='Custom Tools',area='right',w=326,content=widgets['window'],aa='right')
...
def drawCurve2(*args):
	...
	if cmds.nodeType( cmds.ls(sl=1) ) != 'aiStandard':
		cmds.warning('Select an aiStandard Material!')
		return False

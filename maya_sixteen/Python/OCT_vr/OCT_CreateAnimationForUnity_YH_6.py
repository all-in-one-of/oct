...
class CreateAnimationForUnity():
    ...
    def show(self):
        ...
        mc.rowLayout('offsetRow', numberOfColumns = 6, columnAttach6 = ['left', 'left', 'left','left', 'left', 'left'], columnWidth6 = [50,50,50,50,50,50], columnOffset6 =[5,5,5,5,5,5], adjustableColumn6 = True, parent = 'First_Set')
        mc.text(label=u'旋转偏移x轴：', w = 70, parent = 'offsetRow')
        mc.textField('offsetX', text = '0', width = 50, alwaysInvokeEnterCommandOnReturn= True, parent = 'offsetRow')
        mc.text(label=u'旋转偏移y轴：', w = 70,parent = 'offsetRow')
        mc.textField('offsetY', text = '0', width = 50, alwaysInvokeEnterCommandOnReturn= True, parent = 'offsetRow')            
        mc.text(label=u'旋转偏移z轴：', w = 70,parent = 'offsetRow')
        mc.textField('offsetZ', text = '0', width = 50, alwaysInvokeEnterCommandOnReturn= True, parent = 'offsetRow') 
        
        mc.rowLayout('checkRow', parent = 'First_Set')       
        mc.checkBox("setZero",label=u"设置高度为0（旋转木马项目需勾去掉）",v=1, parent = 'checkRow')
...
#选择拾取动态库文件
    ...
    def CreateAni(self, *args):
        offtX = float(mc.textField('offsetX', query=True, text = True, alwaysInvokeEnterCommandOnReturn= True))
        offtY = float(mc.textField('offsetY', query=True, text = True, alwaysInvokeEnterCommandOnReturn= True))
        offtZ = float(mc.textField('offsetZ', query=True, text = True, alwaysInvokeEnterCommandOnReturn= True))
                ...
                if FrameFL:
                    ...
                    if FrameNum%4==0:
                        ...
                        if rotaFV:
                            ...
                            mc.rotate((float(rotaFL[0])+offtX)*checkHi, -float(rotaFL[1])+offtY, float(rotaFL[2])+offtZ, self._myCube, a=True) #加偏移值，设置x轴

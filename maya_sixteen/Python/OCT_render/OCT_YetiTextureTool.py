...
class YetiFile_Tools(QtGui.QDialog):
    def __init__(self,fileType, nodeAttr, dirName, parent=getMayaWindow()):
        super(YetiFile_Tools, self).__init__(parent)
            ...
            while(True):
                ...
                for key in myCacheFileDirt.keys():
                    ...
                    if os.path.isdir(key) or os.path.isfile(key):
                        ...
                        #print lastCacheFile
                        frontCachePath = ''
                        splitTexts = lastCacheFile.split('\\')
                        #创建目录
                        for i, Text in enumerate(splitTexts):
                            if i == 0:
                                frontCachePath = root
                            if i <= len(splitTexts)-2 and os.path.isfile(key):
                        ...
                        targetFile = self.myChangeNetPath(frontCachePath)
                        textures = []
                        if os.path.isdir(key):
                            textures  = os.listdir(sourceFile)
                        else:
                            sourceFile = os.path.dirname(sourceFile)
                            textures.append(os.path.basename(key))
                        for tex in textures:
                            oldFilePath = os.path.join(sourceFile, tex)
                            newFilePath = os.path.join(targetFile, tex)
                            if not os.path.isfile(newFilePath):
                                self.worker.ready(oldFilePath, newFilePath)
                                while True:
                                    if self.worker.wait():
                                        break
                                    else:
                                ...
                                if os.path.getmtime(oldFilePath) != os.path.getmtime(newFilePath):
                                    self.worker.ready(oldFilePath, newFilePath)
                                    while True:
                                        if self.worker.wait():
                                            break
                        ...
                        for eachfile in myCacheFileDirt[key]:
                            if os.path.isfile(key):
                                mc.setAttr('%s.%s' % (eachfile, self.nodeAttr), newFilePath, type='string')
                            else:
    ...
    def soreFilePath(self, allCacheFile):
        ...
        for eachfile in allCacheFile:
            ...
            if not cachePath:
                cachePath = 'scene\\'
                mc.setAttr('%s.%s' % (eachfile, self.nodeAttr), cachePath, type='string')
...
if __name__ == "__main__":
    ...
    File_Ui = YetiFile_Tools(fileType, nodeAttr, dirName)
    File_Ui.show()
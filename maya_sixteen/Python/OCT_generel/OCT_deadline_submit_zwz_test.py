...
class AssetDeadline():
    ...
    def selectServer(self, *args):
        ...
        if addr == '':
            ...
            try:
                str = os.popen(r'"%s/deadlinecommand.exe" -ChangeRepository %s/DeadlineRepository8' % (addr, self.deadlineSereveIp)).read()
            except:
...
class CopyProject(QtGui.QDialog):
    ...
    def myCreateDeepFolder(self, address):
        try:
            ...
            print address
            cmd = r'%s -u %s -p %s -hide -wait -c -nowarn -ex "md %s"' % (self.CpauLocalPath, r'octvision\rd', r'rd1234', address)
            print cmd
            p = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
            while True:
                if not p.poll() is None:
    ...
    def main(self, *args):
        ...
        if self.serveProject:
            ...
            if copyType == 2 or copyType == 3 or copyType == 4:
                ...
                mm.eval('source "SubmitMayaToDeadline_YH";')
                mm.eval('SubmitMayaToDeadline_YH')

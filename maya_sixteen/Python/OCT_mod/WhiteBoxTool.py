...
def makeWin():
    ...
    mc.text(label = 'Please select the initial model to be duplicated and click the"OCT_mod.WhiteBoxTool.store source model" button', ww=True, al='center')
    mc.button(label= 'Store Source Model', align='right', command='OCT_mod.WhiteBoxTool.storeTarget()')
    mc.checkBox(label='Match Targets Translation?', onc='OCT_mod.WhiteBoxTool.matchTranslate()', ofc='OCT_mod.WhiteBoxTool.dontMatchTranslate()', v=True)
    mc.checkBox(label='Match Targets Scale?', onc='OCT_mod.WhiteBoxTool.matchScale()', ofc='OCT_mod.WhiteBoxTool.dontMatchScale()', v=True)
    mc.checkBox(label='Match Targets Rotation?', onc='OCT_mod.WhiteBoxTool.matchRotation()', ofc='OCT_mod.WhiteBoxTool.dontMatchRotation()', v=True)
    mc.checkBox(label='Instance Source Model?', onc='OCT_mod.WhiteBoxTool.OpenInstanceModel()', ofc='OCT_mod.WhiteBoxTool.CloseInstanceModel()', bgc=(0.5, 0.9, 0.5), v=True)
    ...
    mc.button(label= 'Replace Targets', align='right', command='OCT_mod.WhiteBoxTool.replaceTargets()')
    mc.button(label= 'Help', command='OCT_mod.WhiteBoxTool.helpWin()')
    mc.button(label= 'Cancel', command='OCT_mod.WhiteBoxTool.cancel()')
    #Call window
...
def OpenInstanceModel():
    global mIns_zwz
    mIns_zwz = 1

def CloseInstanceModel():
    global mIns_zwz
    mIns_zwz = 0
...
def replaceTargets():
    ...
    spcialFlag = False
    if sourceObject:
        myOrigalShape = mc.listRelatives(sourceObject[0], c=True)[0]
        try:
            MeshVC = mc.listConnections('%s.inMesh' % myOrigalShape, s=True)
        except:
            pass
        else:
            print MeshVC
            print 1
            myOrigalType = mc.nodeType(MeshVC[0])
            if myOrigalType == 'transformGeometry':
                spcialFlag = True
        if not spcialFlag:
            myAllHistorys = mc.listHistory(sourceObject[0])
            for myHistory in myAllHistorys:
                if mc.nodeType(myHistory) == 'cacheFile':
                    spcialFlag = True
                    break


    if targetObjects and sourceObject:
        for x in range (numberOfTargets):
            mc.select(targetObjects[x])
            myPosition = mc.xform(targetObjects[x], q=True, rp=True, ws=True)
            mc.xform(cp=True) 
            #Duplicate the source object
            if mIns_zwz == 1:
                sO = mc.instance(sourceObject[0])
            else:
                if spcialFlag:
                    sO = mc.duplicate(sourceObject[0], un=True)
                else:
                    sO = mc.duplicate(sourceObject[0])
            mc.select(sO)
            mc.xform(cp=True)
            ...
            mc.xform(sO[0], rp=myPosition, ws=True)
    else:
        mc.warning(u'没有选择到任何物体')

...
class AssetDeadline():
    ...
    def show(self, *args):
        ...
        messageRadio = mc.radioButtonGrp('messagRadioBG', label=u'查看Deadline服务器信息', labelArray2=[u'是', u'否'], sl=2,
                                         cl3=['left', 'left', 'left'], cw3=[150, 100, 100], numberOfRadioButtons=2,
                                         p=oneC)
        mc.radioButtonGrp('AutoSubmit', label=u'是否全自动提交Deadline', labelArray2=[u'是', u'否'], sl=2,
                          cl3=['left', 'left', 'left'], cw3=[150, 100, 100], numberOfRadioButtons=2, p=oneC)
        mc.radioButtonGrp('ArnoldProxy', label=u'aiStandIn', labelArray2=[u'是', u'否'], sl=1,
                          cl3=['left', 'left', 'left'], cw3=[150, 100, 100], numberOfRadioButtons=2, p=oneC)
        ...
        mc.formLayout(form, edit=True,
                      attachForm=[(one, 'left', 5), (two_one, 'right', 5), (two_two, 'right', 5), (one, 'top', 84),
                                  (two_one, 'top', 84), (three_one, 'left', 5), (three_one, 'right', 5),
                                  (three_two, 'left', 5), (three_two, 'right', 5), (three_two, 'bottom', 5)],
                      attachControl=[(two_one, 'left', 1, one), (two_two, 'left', 1, one), (two_two, 'top', 1, two_one),
                                     (one, 'bottom', 1, three_one), (two_two, 'bottom', 1, three_one),
                                     (three_one, 'bottom', 1, three_two)],
    ...
    def selectServer(self, *args):
        ...
        if addr == '':
            ...
            try:
                str = os.popen(r'"%s/deadlinecommand.exe" -ChangeRepository %s/DeadlineRepository10' % (
                addr, self.deadlineSereveIp)).read()
            except:
                ...
                if mValue == 1:
                    try:
                        str = os.popen(
                            r'"%s/deadlinecommand.exe" -executescript //octvision.com/cg/Tech/maya/2013/Python/OCT_generel/Deadline/getServerInfo.py' % addr).read()
                    except:
                        mc.confirmDialog(title=u'温馨提示：', message=u'获取Deadline的信息失败，请联系技术管理员！', button=['OK'],
                                         defaultButton='Yes', dismissString='No')
                ...
                if self.deadlineSereveIp == r'//192.168.90.163':
                    ...
                    try:
                        address = os.popen(
                            r'"%s/deadlinecommand.exe" -executescript //octvision.com/cg/Tech/maya/2013/Python/OCT_generel/Deadline/getImagesPath.py' % addr).read()
                    except:
                        pass
                    else:
                        minIndex = 0
    ...
    def checkFile(self, copyType):
        ...
        if not myAllCameras:
            ...
            mc.confirmDialog(title=u'温馨提示：', message=u'无可渲染的摄像机！请设置需要渲染的摄像机', button=['OK'], defaultButton='Yes',
                             dismissString='No')
            return False
        ...
        if copyType != 1:
            if self.fileSName:
                    ...
                    if os.path.isdir(serFilePath):
                        if mc.file(q=True, amf=True):
                            if self.copyType == 2 or self.copyType == 4:
                                saveFlag = mc.confirmDialog(title=u'温馨提示', message=u'文件已被修改过，请问是否先保存再继续提交？',
                                                            button=['Yes', 'SaveAs', 'No'], defaultButton='Yes',
                        ...
                        mc.confirmDialog(title=u'警告',
                                         message=u'在\\octvision.com\cg\Themes下不存在相应工程！\n--------请检查文件命名是否正确！--------',
                                         button=['OK'], defaultButton='Yes', dismissString='No')
                        return False
                else:
                    mc.confirmDialog(title=u'警告',
                                     message=u'在\\octvision.com\cg\Themes下不存在相应工程！\n--------请检查文件命名是否正确！--------',
                ...
                if nameFlag:
                    ...
                    if len(fileSNew) >= 3:
                        # 判断服务器是否存在该工程
                        serFilePath = os.path.join(PROJECT_PATH, fileSNew[0], r'Project\scenes\animation', fileSNew[1],
                                                   fileSNew[2])
        ...
        if mc.pluginInfo("mtoa.mll", q=True, loaded=True) and mc.getAttr(
                "defaultRenderGlobals.currentRenderer") == "arnold":
            mc.setAttr("defaultRenderGlobals.imageFilePrefix", "<Scene>/<RenderLayer>/<Camera>/<RenderPass>/<Camera>",
                       type="string")
        ...
        if AllIsParFlag:
            ...
            if os.path.isdir(parPath):
                ...
                if not FindParStartFlag:
                    saveFlag = mc.confirmDialog(title=u'警告：', message=u'文件中的粒子没有相应的Startup文件，必须进行保存！',
                                                button=[u'保存', u'中断退出'], defaultButton='Yes', cancelButton='No',
                                                dismissString='No')
                    if saveFlag == u'\u4fdd\u5b58':
                        mm.eval("SaveScene")
                        time.sleep(1)
                        ...
                        try:
                            ...
                            if myFileName:
                                ...
                                if self.copyType == 1 and not FileOkFlag:
                                    ...
                                    for i in proxyPaths:
                                        path = os.path.join(OCT_ArnoldPathNew, i, dirName, r'sourceimages',
                                                            r'arnoldtex', proxyName)
                                        path = path.replace('\\', '/')
        ...
        if mc.pluginInfo('vrayformaya.mll', query=True, loaded=True):
            ...
            noVrIrrMapFiles = myCheck_VrSetFilesModel(VrIrrMap_typeOn, VrIrrMap_typeModel, VrIrrMap_typeFuleAttr,
                                                      VrIrrMap_numModel, VrIrrMapSets)
            # Light cache map光子贴图
            ...
            noVrLightCFiles = myCheck_VrSetFilesModel(VrLightC_typeOn, VrLightC_typeModel, VrLightC_typeFuleAttr,
                                                      VrLightC_numModel, VrLightCSets)
            # Caustics的焦散贴图
            ...
            noVrCausticsFiles = myCheck_VrSetFilesModel(VrCaustics_typeOn, VrCaustics_typeModel,
                                                        VrCaustics_typeFuleAttr, VrCaustics_numModel, VrCausticsSets)
            # 统计错误
        ...
        numNoFiles = (
                    numnoTexFiles + numnoArnoldTxFiles + numNoCacheFiles + numWrongCacheFiles + numNoYetiCacheFiles + numWrongYetiTexFiles + numNoYetiTexFiles + numNoAbcCacheFiles +
        ...
        if ErrorText:
            ...
            mc.confirmDialog(title=u'警告', message=u'%s' % ErrorText, button=['OK'], defaultButton='Yes',
                             dismissString='No')
            try:
                                        ...
                                        if len(IndexPathSplitT) > 1:
                                            ...
                                            if IndexLowerPathType != '.hdr' and not UseSeqFlag and IndexLowerPathType != '.tx':
                                                ...
                                                if os.path.isfile(
                                                    ...
                                                    texFileNameGroup.append(IndexArnoldTxFileName)
                                                    print("append at =========2=========={}".format(
                                                        IndexArnoldTxFileName))
                                            else:
                                                if not CopyHdrFlag:
                                                    texFileNameGroup.append(IndexTexName)
                                ...
                                if os.path.isdir(serFileName):
                                    if os.path.isfile(serFinalTexFileName):
                                        ...
                                        if int(tmpMtime) >= int(testMtime):
                                            tmpCopyFlag = False
                                            print(
                                                "target file texture {} alread exists and it not old relative to the source file !!!!! ".format(
                                                    serFinalTexFileName))
                            else:
                                tmpCopyFlag = False
    ...
    def main(self, *args):
        ...
        if self.serveProject:
            ...
            if copyType == 2 or copyType == 3 or copyType == 4:
                ...
                mm.eval('source "SubmitJobToDeadline10";')
                mm.eval('SubmitJobToDeadline10')

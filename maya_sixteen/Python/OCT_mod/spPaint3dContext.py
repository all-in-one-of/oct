...
class paintContext (object):
    ...
    def onPress(self):
        ...
        # pressPositionList = []
        # for i in range(8):
        #     _X = round(rand.uniform(pressPosition[0]+4), pressPosition[0]-4), 3)
        #     _Y = round(rand.uniform(pressPosition[1]+3, pressPosition[1]-3), 3)
        #     pressPositionList.append([_X, _Y])
        # if pressPositionList:
        #     for pressPositions in pressPositionList:
        #         worldPos, worldDir = getViewportClick(pressPositions[0],pressPositions[1])
    ...
    #Judge origal object whether need to special duplicate
    def judgeOrigalSDFlag(self, object):
        spcialFlag = False
        myOrigalShape = mc.listRelatives(object, c=True)[0]
        try:
            ...
            if myOrigalType == 'transformGeometry':
                spcialFlag = True
        if not spcialFlag:
            myAllHistorys = mc.listHistory(object)
    ...
    def createObject(self,intersection):
        ...
        if (self.uiValues.instance):
            ...
            tempDAG = mc.listRelatives(intersection.dagMeshSourceObject, parent=True)
            #change by zwz,To support vary and Arnold proxy
            spcialFlag = self.judgeOrigalSDFlag(tempDAG[0])
            if not spcialFlag:
                newObjectDAG = mc.duplicate(tempDAG[0], ic=self.uiValues.preserveConn)
            else:
                newObjectDAG = mc.duplicate(tempDAG[0], un=True, ic=self.uiValues.preserveConn)
    ...
    def fetchCursorObject(self):
        ...
        if (self.uiValues.instance):
            ...
            if not spcialFlag:
                ...
                newObjectDAG = mc.duplicate(sourceDAG, un=True, ic=self.uiValues.preserveConn)

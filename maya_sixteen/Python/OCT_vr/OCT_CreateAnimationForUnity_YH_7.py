...
class CreateAnimationForUnity():
    ...
    def show(self):
        ...
        two =mc.frameLayout('Subsection_set',label = u'-------------------------------------------分段设置-------------------------------------------',labelAlign = 'top',borderStyle = 'etchedOut',w=405)
        mc.rowLayout('fourRow', numberOfColumns = 3,columnAttach3 = ['left', 'left', 'left'],columnWidth3 = [10, 255, 200], columnOffset3 =[2, 2, 2], adjustableColumn3 = True, parent = 'Subsection_set')
        mc.text(l = u'每段起始帧', parent = 'fourRow')
        mc.text(l = u'头尾融合帧', parent = 'fourRow')
        mc.text(l = u'距离缩放', parent = 'fourRow')
    ...
    def Subsection(self):
        ...
        if self._textSubT!="":
            ...
            for i in range(num):         	
                ...
                if i < 1 :
                   mc.textField('fuse%s'%i, tx = 0, h = 20,parent = 'fiveRow%s'%i)
                else :
                   mc.textField('fuse%s'%i, tx = i/i*4, h = 20,parent = 'fiveRow%s'%i) #设置融合值倍率，为偶数
                mc.floatSliderGrp('scale%s'%i, v = 1, h = 20, field = True, min = 0.1, max = 2, parent = 'fiveRow%s'%i)

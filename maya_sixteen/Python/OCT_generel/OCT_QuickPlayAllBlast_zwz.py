...
class QuickPlayAllBlast_C_zwz():
    ...
    def show(self):
        ...
        mc.rowLayout('PresetRow', numberOfColumns=2, columnAttach2=['left', 'left'], columnWidth2=[60, 68], columnOffset2=[2, 2], adjustableColumn2=True, parent='First_Set')
        mc.text(l = 'Preset:', parent='PresetRow')
        mc.checkBox('Preset', l=u'180度球幕', v=0, parent='PresetRow')
    ...
    def SetRenderStarFram_zwz(self, *args):
        ...
        mc.setAttr("defaultRenderGlobals.extensionPadding", 5)
    ...
    def MyWindows_model_zwz(self, myWinName, myCamera):
        ...
        mc.paneLayout(myPaneLayoutName, w=int(_width)+4, h=int(_Height)+5, configuration='single', aft=0, st=1)
    ...
    def OptimizeWin(self, *args):
        ...
        for myModePlane in self.myAllModePlanes:
            mc.modelEditor(myModePlane, e=True, polymeshes=True, nurbsCurves=False, nurbsSurfaces=False, subdivSurfaces=False,
                   ...
                   textures=False, strokes=True, motionTrails=False, manipulators=False, clipGhosts=False, grid=False, hud=False, rendererName = rendererName)
    ...
    def CreateMyWindows_zwz(self, *args):
        ...
        self.myAllWindows = []
        self.myAllModePlanes = []
        self.Preset = mc.checkBox('Preset', q=True, v=True)
    ...
    def SavePic_zwz(self, myModelPlane, Frame):
        ...
        view.refresh(False, False, False)
        ...
        if not os.path.isdir(myPath):
            os.makedirs(myPath)
        fileName = str('%s\\%s.%05d.%s' % (myPath, myCameraShortName, Frame, self.TexModel)).encode('gb2312')
        img.writeToFile(fileName, self.TexModel)
        view.refresh(True, True, False)

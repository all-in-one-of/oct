...
def OCT_RenderLayers_Menum_zwz():
    ...
    mc.rowColumnLayout('Xml_First_Com', numberOfColumns=7, columnWidth=[(1, 55), (2, 50), (3, 50), (4, 40), (5, 40), (6, 50), (7, 50)], h=33)
    ...
    mc.button(label='ColorAG', width=50, h=30, command='OCT_render.OCT_RenderLayers_zwz.CreateGamma().AssignVRayGamma()', backgroundColor=(0.5, 0.774, 0.774))
    ...
    mc.setParent('mainmenu')
    mc.frameLayout('secondFLs', label=u'以下层都带置换，透明等 注：物体赋新RGBA材质后尽量不重复选择', labelAlign='top', borderStyle='etchedOut')
    mc.columnLayout('Xml_RGBA___Tran', adjustableColumn=True)
    mc.rowLayout('txRow', numberOfColumns=9, columnAttach6=['left', 'left', 'left', 'left', 'left', 'left'], columnWidth6=[55, 55, 55, 55, 55, 55], columnOffset6 =[2, 2, 2, 2, 2, 2], h=33)
    mc.button(label='R', width=55, h=30, command='OCT_render.OCT_RenderLayers_zwz.AssignSurMatte_Tran(0)', backgroundColor=(0.7, 0,  0))
    mc.button(label='G', width=55, h=30, command='OCT_render.OCT_RenderLayers_zwz.AssignSurMatte_Tran(1)', backgroundColor=(0, 0.7, 0))
    mc.button(label='B', width=55, h=30, command='OCT_render.OCT_RenderLayers_zwz.AssignSurMatte_Tran(2)', backgroundColor=(0, 0, 0.7))
    mc.button(label='Y', width=55, h=30, command='OCT_render.OCT_RenderLayers_zwz.AssignSurMatte_Tran(5)', backgroundColor=(0.7, 0.7, 0))
    mc.button(label='C', width=55, h=30, command='OCT_render.OCT_RenderLayers_zwz.AssignSurMatte_Tran(6)', backgroundColor=(0, 0.7, 0.7))
    mc.button(label='P', width=55, h=30, command='OCT_render.OCT_RenderLayers_zwz.AssignSurMatte_Tran(7)', backgroundColor=(0.7, 0, 0.7))
    mc.button(label='A', width=55, h=30, command='OCT_render.OCT_RenderLayers_zwz.AssignSurMatte_Tran(3)', backgroundColor=(0.8, 0.8, 0.8))
    mc.button(label='surface', width=55, h=30, command='OCT_render.OCT_RenderLayers_zwz.AssignSur()', backgroundColor=(0.6, 0.6, 0.6))
    mc.button(label='Matte', width=55, h=30, command='OCT_render.OCT_RenderLayers_zwz.AssignSurMatte_Tran(4)', backgroundColor=(0.3, 0.3, 0.3))
    ...
    mc.rowLayout('VRayRow', numberOfColumns=2, columnAttach2=['left', 'left'], columnWidth2=[60, 35], columnOffset2=[1, 1], h=33)
    ...
    #mc.button(label='VRayMtl', w=60, h=30, command='OCT_render.OCT_RenderLayers_zwz.materialChange().convertVray()', backgroundColor=(0.5, 0.774, 0.1))
    #mc.button(label='aiStandard', w=60, h=30, command='OCT_render.OCT_RenderLayers_zwz.materialChange().convertArnold()', backgroundColor=(0.5, 0.774, 0.2))
    #mc.button(label='blinn', w=60, h=30, command='OCT_render.OCT_RenderLayers_zwz.materialChange().convertBlinn()', backgroundColor=(0.5, 0.774, 0.4))
    #mc.button(label='VRayBlendTolayeredShader', w=150, h=30, command='OCT_render.OCT_RenderLayers_zwz.VRayBlendTolayeredShader().VRayBlendMtlTolayeredShader()', backgroundColor=(0.5, 0.774, 0.5))
...
def VraySdtoMySD_zwz(mySd, mySd_SG, vrShd_SG, myCSd, myCSd_SG, allMyShapes=[]):
    ...
    elif myInputShaderType==6:
        SdTransAttr='outTransparency'

    for each in vrShd_SG:
        if myInputShaderType != 2 and myInputShaderType != 6:
        ...
        MyRGBObjects = []
        tmp = ''
        if myInputShaderType == 2 or myInputShaderType==6:
        ...
        if SGC_V == 1 or TrC_V == 1 or TrV_V == 1 or BuC_V == 1 or LaC_V == 1:
            #复制材质球
            if myInputShaderType == 0 or myInputShaderType == 1 or myInputShaderType == 2 or myInputShaderType == 4 or myInputShaderType == 5 or myInputShaderType==6:
            ...
            if TrC_V == 1:
                ...
                if opcityType == 'file':
                    ...
                    if myInputShaderType == 2 or myInputShaderType == 5 or myInputShaderType==6:
                ...
                #具有透明值
                if TrV_V == 1:
                    if not (myInputShaderType == 2 or myInputShaderType == 5 or myInputShaderType==6):
                        mc.editRenderLayerAdjustment('%s.%s'%(tranc_Point,SdTransAttr))
                    if myInputShaderType == 1 or myInputShaderType == 2 or myInputShaderType == 3 or myInputShaderType == 4 or myInputShaderType == 5 or myInputShaderType==6:
...
def ChangeGammaCorrectValue_zwz(i):
    if i==1:
        ...
        # allUGammaCorrects = [eaMGC for eaMGC in mc.ls(type='gammaCorrect') if eaMGC.find('MyGammaCorrect') != -1]
    else:
        GammaValue =mc.textField('GCVRayValue', query = 1, text = 1)
        allUGammaCorrects= mc.ls('*MyVRayGammaCorrect*', type='gammaCorrect')
        # allUGammaCorrects = [eaMGC for eaMGC in mc.ls(type='gammaCorrect') if eaMGC.find('MyVRayGammaCorrect') != -1]
...
#surfaceShader材质
def AssignSurMatte_Tran(i):
    slObject=mc.ls(sl=True)
    myRGBALam = myRGBACLam = myRGBACLam_SG =''
    if len(slObject):
        allFShader=SortallSelShader_zwz()
        myRGBALam_out = CreatMyRGBASur()
        myRGBALam = myRGBALam_out[0+i*4]
        myRGBALam_SG = myRGBALam_out[1+i*4]
        myRGBACLam = myRGBALam_out[2+i*4]
        myRGBACLam_SG = myRGBALam_out[3+i*4]
...
def CreatMyRGBASur():
    mc.select(cl = True)
    if not mc.objExists('my_RGBA_surR'):
        myRLam = mc.shadingNode('surfaceShader',asShader =True, n ='my_RGBA_surR')
        myRLam_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myRLam+'_SG')
        mc.connectAttr(myRLam + '.outColor',myRLam_SG + '.surfaceShader',force=True)
        mc.setAttr(myRLam + '.outColor',1,0, 0,type="double3")
        mc.setAttr(myRLam + '.outMatteOpacity',0,0,0,type="double3")
        
    if not mc.objExists('my_RGBA_surR1'):
        tmp = mc.duplicate('my_RGBA_surR',upstreamNodes=1)
        myRLamC = tmp[0]
        myRLamC_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myRLamC+'_SG')
        mc.connectAttr(myRLamC + '.outColor',myRLamC_SG + '.surfaceShader',force=True)
    if not mc.objExists('my_RGBA_surG'):
        myGLam = mc.shadingNode('surfaceShader',asShader =True, n ='my_RGBA_surG')
        myGLam_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myGLam+'_SG')
        mc.connectAttr(myGLam + '.outColor',myGLam_SG + '.surfaceShader',force=True)
        mc.setAttr(myGLam + '.outColor',0,1,0,type="double3")
        mc.setAttr(myGLam + '.outMatteOpacity',0,0,0,type="double3")
    if not mc.objExists('my_RGBA_surG1'):
        tmp = mc.duplicate('my_RGBA_surG',upstreamNodes=1)
        myGLamC = tmp[0]
        myGLamC_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myGLamC+'_SG')
        mc.connectAttr(myGLamC + '.outColor',myGLamC_SG + '.surfaceShader',force=True)
    if not mc.objExists('my_RGBA_surB'):
        myBLam = mc.shadingNode('surfaceShader',asShader =True, n ='my_RGBA_surB')
        myBLam_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myBLam+'_SG')
        mc.connectAttr(myBLam + '.outColor',myBLam_SG + '.surfaceShader',force=True)
        mc.setAttr(myBLam + '.outColor',0, 0,1,type="double3")
        mc.setAttr(myBLam + '.outMatteOpacity',0,0,0,type="double3")
    if not mc.objExists('my_RGBA_surB1'):
        tmp = mc.duplicate('my_RGBA_surB',upstreamNodes=1)
        myBLamC = tmp[0]
        myBLamC_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myBLamC+'_SG')
        mc.connectAttr(myBLamC + '.outColor',myBLamC_SG + '.surfaceShader',force=True)

    if not mc.objExists('my_RGBA_surY'):
        myYLam = mc.shadingNode('surfaceShader',asShader =True, n ='my_RGBA_surY')
        myYLam_SG = mc.sets(renderable=True, noSurfaceShader=True, empty=True, name=myYLam+'_SG')
        mc.connectAttr(myYLam + '.outColor',myYLam_SG + '.surfaceShader',force=True)
        mc.setAttr(myYLam + '.outColor',1, 1, 0, type="double3")
        mc.setAttr(myYLam + '.outMatteOpacity',0,0,0,type="double3")
    if not mc.objExists('my_RGBA_surY1'):
        tmp = mc.duplicate('my_RGBA_surY',upstreamNodes=1)
        myYLamC = tmp[0]
        myYLamC_SG = mc.sets(renderable=True, noSurfaceShader=True, empty=True, name=myYLamC+'_SG')
        mc.connectAttr(myYLamC + '.outColor',myYLamC_SG + '.surfaceShader', force=True)

    if not mc.objExists('my_RGBA_surC'):
        myCLam = mc.shadingNode('surfaceShader',asShader =True, n ='my_RGBA_surC')
        myCLam_SG = mc.sets(renderable=True, noSurfaceShader=True, empty=True, name=myCLam+'_SG')
        mc.connectAttr(myCLam + '.outColor', myCLam_SG + '.surfaceShader',force=True)
        mc.setAttr(myCLam + '.outColor',0, 1, 1, type="double3")
        mc.setAttr(myCLam + '.outMatteOpacity',0,0,0,type="double3")
    if not mc.objExists('my_RGBA_surC1'):
        tmp = mc.duplicate('my_RGBA_surC',upstreamNodes=1)
        myCLamC = tmp[0]
        myCLamC_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myCLamC+'_SG')
        mc.connectAttr(myCLamC + '.outColor', myCLamC_SG + '.surfaceShader',force=True)

    if not mc.objExists('my_RGBA_surP'):
        myPLam = mc.shadingNode('surfaceShader',asShader =True, n ='my_RGBA_surP')
        myPLam_SG = mc.sets(renderable=True, noSurfaceShader=True, empty=True, name=myPLam+'_SG')
        mc.connectAttr(myPLam + '.outColor', myPLam_SG + '.surfaceShader',force=True)
        mc.setAttr(myPLam + '.outColor', 1, 0, 1, type="double3")
        mc.setAttr(myPLam + '.outMatteOpacity',0,0,0,type="double3")
    if not mc.objExists('my_RGBA_surP1'):
        tmp = mc.duplicate('my_RGBA_surP',upstreamNodes=1)
        myPlamC = tmp[0]
        myPlamC_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myPlamC+'_SG')
        mc.connectAttr(myPlamC + '.outColor',myPlamC_SG + '.surfaceShader',force=True)

    if not mc.objExists('my_RGBA_surA'):
        myALam = mc.shadingNode('surfaceShader',asShader =True, n ='my_RGBA_surA')
        myALam_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myALam+'_SG')
        mc.connectAttr(myALam + '.outColor',myALam_SG + '.surfaceShader',force=True)
        mc.setAttr(myALam + '.outColor',0, 0,0,type="double3")
    if not mc.objExists('my_RGBA_surA1'):
        tmp = mc.duplicate('my_RGBA_surA',upstreamNodes=1)
        myALamC = tmp[0]
        myALamC_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myALamC+'_SG')
        mc.connectAttr(myALamC + '.outColor',myALamC_SG + '.surfaceShader',force=True)
    if not mc.objExists('my_RGBA_surM'):
        myMLam = mc.shadingNode('surfaceShader',asShader =True, n ='my_RGBA_surM')
        myMLam_SG = mc.sets(renderable=True, noSurfaceShader=True, empty=True, name=myMLam+'_SG')
        mc.connectAttr(myMLam + '.outColor', myMLam_SG + '.surfaceShader', force=True)
        mc.setAttr(myMLam + '.outColor',0, 0,0,type="double3")
        mc.setAttr(myMLam + '.outMatteOpacity',0,0,0,type="double3")

    if not mc.objExists('my_RGBA_surM1'):
        tmp = mc.duplicate('my_RGBA_surM',upstreamNodes=1)
        myMLamC = tmp[0]
        myMLamC_SG = mc.sets(renderable=True, noSurfaceShader=True,empty=True, name = myMLamC+'_SG')
        mc.connectAttr(myMLamC + '.outColor',myMLamC_SG + '.surfaceShader',force=True)
    myRGBALam_out=['my_RGBA_surR', 'my_RGBA_surR_SG', 'my_RGBA_surR1', 'my_RGBA_surR1_SG',\
                     'my_RGBA_surG', 'my_RGBA_surG_SG', 'my_RGBA_surG1', 'my_RGBA_surG1_SG',\
                     'my_RGBA_surB', 'my_RGBA_surB_SG', 'my_RGBA_surB1', 'my_RGBA_surB1_SG',\
                     'my_RGBA_surA', 'my_RGBA_surA_SG', 'my_RGBA_surA1', 'my_RGBA_surA1_SG',\
                     'my_RGBA_surM', 'my_RGBA_surM_SG', 'my_RGBA_surM1', 'my_RGBA_surM1_SG',\
                     'my_RGBA_surY', 'my_RGBA_surY_SG', 'my_RGBA_surY1', 'my_RGBA_surY1_SG',\
                     'my_RGBA_surC', 'my_RGBA_surC_SG', 'my_RGBA_surC1', 'my_RGBA_surC1_SG',\
                     'my_RGBA_surP', 'my_RGBA_surP_SG', 'my_RGBA_surP1', 'my_RGBA_surP1_SG']
    return myRGBALam_out
...
class CreateGamma():
    ...
    def AssignVRayGamma(self):
        ...
        if len(allSelShader)>0:
            print allSelShader
            MyGammaCorrect =self.CreateGammaCorrect()
            print MyGammaCorrect
            self.ShaderAddCorrect(allSelShader, MyGammaCorrect)
                     
    def CreateGammaCorrect(self):
        MyGammaCorrect=""
        if not mc.objExists('MyGammaCorrect'):
            MyGammaCorrect = mc.shadingNode('gammaCorrect',asUtility =True, n ='MyGammaCorrect')
            mc.setAttr('%s.gammaX'%MyGammaCorrect, 0.454)
    ...
    def ShaderAddCorrect(self,allSelShader, MyGammaCorrect):
        for each in allSelShader:
            mc.select(cl=True)
            type=""
            try:
                ...
                if type=='blinn' or type=='lambert' or type=="phong" or type=="phongE":
                    try:
                        flag=True
                        myLColorOutV = mc.listConnections('%s.color'%(each), s=True, d=False, plugs=True)
                        if myLColorOutV and myLColorOutV[0].find('GammaCorrect') < 0:
                            MyGammaCorrects=mc.duplicate(MyGammaCorrect, upstreamNodes=1)[0]
                            mc.disconnectAttr(myLColorOutV[0],'%s.color'%each)
                            mc.connectAttr(myLColorOutV[0],"%s.value"%MyGammaCorrects)
                            mc.connectAttr('%s.outValue'%MyGammaCorrects,'%s.color'%each)
                            flag=False
                 
                        if flag:
                            myColorV = mc.getAttr('%s.color'%each)[0]
                            if myColorV != (1.0, 1.0, 1.0) and myColorV != (0.0, 0.0, 0.0):
                                MyGammaCorrects = mc.duplicate(MyGammaCorrect, upstreamNodes=1)[0]
                                myColorV = mc.getAttr('%s.color'%each)[0]
                                mc.setAttr("%s.value" % MyGammaCorrects, myColorV[0], myColorV[1], myColorV[2], type="double3")
                                mc.connectAttr('%s.value'%MyGammaCorrects, '%s.color'%each)
                    except:
                        pass
                       
                        
                elif type=="mia_material" or type=="mia_material_x":
                    try:
                        flags=True
                        myLColorOutV = mc.listConnections('%s.diffuse'%each, s=True, d=False, plugs=True)
                        if myLColorOutV and myLColorOutV[0].find('GammaCorrect') < 0:
                            MyGammaCorrects=mc.duplicate(MyGammaCorrect, upstreamNodes=1)[0]
                            mc.disconnectAttr(myLColorOutV[0],'%s.diffuse'%each)
                            mc.connectAttr(myLColorOutV[0],"%s.value"%MyGammaCorrects)
                            mc.connectAttr('%s.outValue'%MyGammaCorrects,'%s.diffuse'%each)
                            flags=False
                        myLColorOutVA = mc.listConnections('%s.diffuseA'%(each), s=True, d=False, plugs=True)
                        if myLColorOutVA and myLColorOutV[0].find('GammaCorrect') < 0:
                            mc.disconnectAttr(myLColorOutVA[0],'%s.diffuseA'%each)
                       
                        if flags:
                            myColorV = mc.getAttr('%s.diffuse'%each)[0]
                            if myColorV != (1.0, 1.0, 1.0) and myColorV != (0.0, 0.0, 0.0):
                                MyGammaCorrects = mc.duplicate(MyGammaCorrect, upstreamNodes=1)[0]
                                myColorV = mc.getAttr('%s.diffuse'%each)[0]
                                mc.setAttr("%s.value" % MyGammaCorrects, myColorV[0], myColorV[1], myColorV[2], type="double3")
                                mc.connectAttr('%s.value'%MyGammaCorrects, '%s.diffuse'%each)
                    except:
                        pass